...
문서의 지식그래프에서 핵심 주제를 중심으로 Triple을 묶고, Metadata로 문맥을 보강해 검색에 사용할 표현을 생성합니다.
...
원문의 `"이 공정"`처럼 해당 청크만으로 의미가 충분하지 않은 표현에 문서의 Metadata를 함께 제공해 검색에 필요한 맥락을 보강합니다. 이를 통해 질의와 원문의 표현이 직접 일치하지 않더라도 관련 원문을 찾을 수 있도록 합니다.

## 동작 방식

1. 문서 지식그래프에서 중요도가 높은 핵심 주제를 고릅니다.
2. 같은 주제를 설명하는 Triple을 묶습니다.
3. Triple과 문서 Metadata를 LLM에 제공해 검색표현을 생성합니다.
4. 생성된 검색표현을 근거가 된 원문 청크와 연결합니다.

Metadata는 검색표현에 문맥을 보강하는 데 사용되며, 검색표현 자체는 답변의 사실 근거가 아닙니다.
...
| `g2.triple_token_budget`                     | 6000                                               | 한 번에 모델에 제공하는 Triple의 토큰 상한 |
| `g2.minimum_distinct_triples_per_expression` | 1                                                  | 검색표현 하나에 필요한 최소 Triple 수    |
| `g2.metadata_priority`                       | 정의 파일과 동일                                          | 문맥 보강에 사용할 Metadata 우선순위    |

검색표현의 생성 방식이나 Triple을 묶는 기준을 변경하면 생성 결과가 달라지므로 검색표현 생성 이후 단계를 다시 처리해야 합니다([실행과 재처리](rerun.md)).
...
| 확인할 것              | 정상                      |
| ------------------ | ----------------------- |
| 검색표현               | 문서에 대한 표현이 생성되어 있습니다    |
| `triple_ids`       | 근거가 된 Triple이 연결되어 있습니다 |
| `source_chunk_ids` | 원문 청크가 연결되어 있습니다        |
