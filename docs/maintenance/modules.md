...
이 페이지는 기존 구현을 다른 구현으로 바꾸는 경우와 새로운 인덱싱 단계를 추가하는 경우를 구분해 설명합니다.
...
3. `python -m pytest -q tests/unit/query tests/unit/config`와 한 문서 E2E를 실행해 API와 평가 진입점이 같은 구현을 사용하는지 확인합니다.
...
7. `python -m pytest -q tests/unit/ingest tests/unit/config`와 `struct4search-smoke-e2e`를 실행합니다.

최적화된 2,567문서 최종 실행 경로도 유지하는 경우에만 `backend/struct4search/orchestration/dag.py`의 `F400_INGESTION_DAG`를 함께 수정합니다. 일반 `struct4search-ingest` 실행 순서는 `ingest/front.py`, `ingest/back.py`와 두 orchestration adapter가 정본입니다.
...
| 최적화된 2,567문서 최종 실행 DAG | `backend/struct4search/orchestration/dag.py` · `F400_INGESTION_DAG` |
