...
접속 문자열과 API key는 Git에 넣지 않습니다. `.env.example`을 `.env`로 복사해 값을 채우면 `struct4search-*` 명령이 시작할 때 자동으로 읽습니다. 이미 export한 값은 유지합니다. 외부 GPT provider를 선택한 profile에서만 `OPENAI_API_KEY`가 필요합니다.
